...
def mainpage(SITE):
    ...
    qa_html = ''
    for q in qa:
        qa_html += f'<div><a target="blank" href="/system/qa/{q["project_id"]}"><b>{q["project_name"]}</b>, {q["date"]}, <i>{q["question"]}</i> => {q["name"]}</a></div>'
    ...
    for fv in fav:
        ...
        fav_html += f'<div class="mp_mb"><a target="blank" href="/system/favorites/{fv["project_id"]}"><b>{fv["project_name"]}</b>, {fv["date_l"]}, <br>{fav_str}<br></a></div>'
