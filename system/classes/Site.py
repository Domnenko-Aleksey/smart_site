...
class Site:
    ...
    def model_init(self):
        self.model = {
            'project_id': False,  # Текущий id проекта
            'items_list': False,  # Список всех топиков проекта полученный из БД
            'x': [],  # Список обработанных BERT - векторов, после завершения обработки - переводим в массив np
            'y': [],  # Список классов, после завершения обработки - переводим в массив np
            'items_count': 0,
            'item_current': 0,
            'epochs_count': 150,
            'epoch_current': 0,
