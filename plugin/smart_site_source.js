...
DA = {
	...
	url_get_intro: '/api/get_intro_ajax',  // Получаем заголовок / проверка на работу системы
	...
	get_intro(callback) {
		...
		let url = 'https://' + DA.domain + DA.url_get_intro;
		DA.ajax(url, form, function(data) {
			// console.log(data);
			DA.intro_text = data.intro_text;
			DA.intro_speech = data.intro_speech;
			callback();
		})
	},
	...
	create_tab() {
			...
			'<div id="da_left">' +
				...
				'<div id="da_speech_container">' +
					'<div class="da_speech_wrap da_w1440">' +
						'<div id="da_mic_icon" class="da_speech_icon" title="Вкл/выкл микрофона">' + DA.icons.microphone + '</div>' +
						'<div id="da_check_icon" class="da_speech_icon display_none" title="Добавить в избранное">' + DA.icons.check + '</div>' +
						'<div id="da_speak_icon" class="da_speech_icon active" title="Вкл/выкл звук">' + DA.icons.audio + '</div>' +
						'<div id="da_speech_out">' + DA.intro_text + '</div>' +
					'</div>' +
				'</div>' +
        ...
        DA.speech.init();  // При открытии вкладки инициализируем SpeechRecognition
	},
	...
	set_active() {
		...
		if (DA.open_icon.classList.contains('active')) {
			if (DA.first_start && DA.speech.sn_on && DA.intro_speech != '') {
				DA.speech.synthesis_start(DA.intro_speech);  // Проговаривает вступление, после которого автоматически стартует распознавание
			} else {
                DA.speech.recognize_start();  // Распознавание речи, если нет вступления
            };
	...
	get_speech_answer_ajax(speech_text, id=false) {
		...
		// При ответе - убираем класс 'display_none' у кнопки избнанное
		if (document.getElementById('da_check_icon').classList.contains('display_none')) {
            document.getElementById('da_check_icon').classList.remove('display_none');
        };
        ...
        document.getElementById('da_answer_content').innerHTML = DA.icons.spinner;
	},
...
// --- AJAX ---
DA.ajax = function(url, form, callback){
...
class DA_show{
	...
	prev(){
		this.num--;
		if(this.num < 0) {
            this.num = this.img_arr.length - 1;
        };
